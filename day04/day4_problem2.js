...
async function processLineByLine() {
  ...
  for (let calledNumberString of calledNumbers) {
    ...
    // Now we remove all the winning boards
    for (let board of winningBoards) {
      bingoBoards.splice(bingoBoards.indexOf(board), 1);
    }
    if (bingoBoards.length === 0) {
      return { winningNumber: calledNumber, winningBoard: winningBoards[0], unmarkedSum: winningBoards[0].unmarkedSum() };          
    }
  }
