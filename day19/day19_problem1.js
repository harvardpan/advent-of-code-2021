...
async function processLineByLine() {
  ...
  // Now that we've rotated all points, we can obtain the total number of unique points.
  let uniqueBeacons = new Set();
  for (let i = 0; i < scanners.length; ++i) {
    for (let j = 0; j < scanners[i].length; ++j) {
      let key = `${scanners[i][j].x},${scanners[i][j].y},${scanners[i][j].z}`;
      if (uniqueBeacons.has(key)) {
        continue;
      }
      uniqueBeacons.add(key);
    }
  }
  console.log(`There are ${uniqueBeacons.size} unique beacons`);
}
