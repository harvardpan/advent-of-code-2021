  ...
  for (let i = 0; i < 14; ++i) {
    ...
    if (increaseOrDecreaseInstruction === 1) { // this is an increase
      stack.push({ z: increaseOrDecreaseInstruction, x: xIncreaseOrDecrease, y: yIncrease, index: i });
       ...
       modelNumber[matchingInstruction.index] = 1 - matchingInstruction.y - xIncreaseOrDecrease; // maximize this number
       if (modelNumber[matchingInstruction.index] <= 0) {
         modelNumber[matchingInstruction.index] = 1;
       }
       modelNumber[i] = modelNumber[matchingInstruction.index] + matchingInstruction.y + xIncreaseOrDecrease;
    }
  }
  console.log(`Minimum model number: ${modelNumber.join('')}`);
