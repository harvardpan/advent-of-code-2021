...
function findHighestVelocity(topLeft, bottomRight) {
  let lowX = Math.ceil(quadraticRoot(topLeft.x));
  let highX = Math.floor(quadraticRoot(bottomRight.x));
  let highY = Math.abs(bottomRight.y);
  let highestPoint = { x: 0, y: 0 };
  let highestVelocity = { x: 0, y: 0 };
  let highestResult = null;
  for (let x = lowX; x <= highX; ++x) {
    for (let y = x; y <= highY; ++y) {
      ...
      if (result.highest.y > highestPoint.x) {
        highestPoint = result.highest;
        highestVelocity = result.velocity;
        highestResult = result;
      }
    }
  }
  console.log(JSON.stringify(highestVelocity, null, 2));
  console.log(JSON.stringify(highestResult, null, 2));
}
...
async function processLineByLine() {
  ...
  for await (const line of rl) {
    ...
    findHighestVelocity(topLeft, bottomRight);
  }
}
